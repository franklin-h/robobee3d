...
def perch_parab_pet(t, initialPos, tend=500, vdes=0.2):
    ...
    #     s2 = np.sqrt((fpdes[0] - initialPos[0]) ** 2 + (fpdes[1] - initialPos[1]) ** 2)
    #     for i in range(1, len(t)):
    #         pdes[i, 0] = pdes[i - 1, 0] +  (fpdes[0] - initialPos[0])/len(t)
    #         pdes[i, 1] = pdes[i - 1, 1] +  (fpdes[1] - initialPos[1])/len(t)
    ...
    #
    #     for i in range(1, len(t)):
    #         pdes[i, 0] = pdes[i-1, 0] + (fpdes[0] - initialPos[0])/(len(t)-1)
    #         pdes[i, 1] = pdes[i-1, 1] + (fpdes[1] - initialPos[1])/(len(t)-1)
    #         #pdes[i, 2] = initialPos[2] - (Amp/D**2)*(np.sqrt((pdes[i, 0] - initialPos[0])**2 + (pdes[i, 1] - initialPos[1])**2) - D)**2 + Amp
    #         pdes[i, 2] = initialPos[2] + (1/(len(t) - 1)) * (fpdes[2] - initialPos[2]) + 4 * Amp * (1/(len(t) - 1)) * (1 - (1/(len(t) - 1)))
    #
    #         # Optional: feedforward velocity (first derivative)
    #         dpdes[:, i] = np.gradient(pdes[:, i])  # rough numerical; refine if needed
    # return pdes, dpdes, sdes